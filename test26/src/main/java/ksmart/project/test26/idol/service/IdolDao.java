...
public class IdolDao {
	...
	private final String ns = "ksmart.project.test26.idol.service.IdolMapper.";
	
	/*아이돌 전체리스트 띄우는부분*/
	public List<Idol> selectIdolList(){
		// idol 전체 리스트를 리턴 받아서 list 에대입
		List<Idol> list = sqlSessionTemplate.selectList(ns + "selectIdolList");
	...
	public List<Idol> selectIdolOne(int idolId) {
		...
		List<Idol> list = sqlSessionTemplate.selectList(ns + "selectIdolOne", idolId);
	...
	public void updateIdol(Idol idol) {
		...
		sqlSessionTemplate.update(ns + "updateIdol", idol);
	}
	...
	public void insertIdol(Idol idol) {
		...
		// idol 객체 정보를 확인 하고 데이터베이스 수정
		sqlSessionTemplate.insert(ns + "insertIdol", idol);
	}
	...
	public void deleteIdol(int idolId) {
		...
		sqlSessionTemplate.delete(ns + "deleteIdol", idolId);
	}
}
